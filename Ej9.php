    ...
    switch($_GET["action"]){
      ...
      case "dlt":
        if(isset($_POST["btnDelete"])){
          if(isset($_POST["prdid"])){
          $sqlDelete = "Delete * from productos where `prdid` = %d ;";
          DeleteRegistro($sqlDelete);
            redirectWithMessage('Producto Eliminado!', 'Ej8.php?action=dlt&prdid='.$_POST["prdid"]);
          }
        }
        break;
      default:
  ...
  <body>
    ...
    <form action="Ej9.php?action=<?php echo $_GET["action"]; ?>" method="POST">
        <?php
         ...
         <input type="submit" name="btnDelete" id="btnDelete" value="Borrar" />
    </form>
  </body>
</html>